...
func setProgrammedCondition(newGw *gatewayv1.Gateway, xdsErr error, pendingACK bool) {
	...
	case pendingACK:
		// Config has been pushed to Envoy but we are still waiting for its ACK.
		programmedCondition.Status = metav1.ConditionUnknown
		programmedCondition.Reason = "Pending"
		programmedCondition.Message = "Waiting for Envoy to confirm the configuration"
	default:
		// Envoy has ACKed the current config. Check if all individual listeners were programmed.
