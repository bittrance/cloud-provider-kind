		...
		for _, listener := range listeners {
			...
			case gatewayv1.HTTPProtocolType, gatewayv1.HTTPSProtocolType:
				...
				for _, httpRoute := range routesByListener[listener.Name] {
					routes, hcmFilterConfigs, validBackendRefs, notAccepted, resolvedRefsFailure, partiallyInvalidCond := translateHTTPRouteToEnvoyRoutes(httpRoute, c.serviceLister, c.referenceGrantLister)
					...
					for i := range hcmFilterConfigs {
						...
						// Only the first filter per type is installed in the listener's HCM.
